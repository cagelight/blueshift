...
blueshift::protocol::~protocol() {
	if (reqq.processing_token) mi.cleanup(reqq.processing_token);
}
...
blueshift::protocol::status blueshift::protocol::do_res_process() {
	...
	if (general_buffer.size()) {
		if (mi.process(reqq.processing_token, req, general_buffer) == module::mss::proceed) {
...
blueshift::protocol::status blueshift::protocol::do_res_multipart() {
	...
	switch (mmode) {
		...
		case multipart_mode::body: {
			if (general_buffer.size()) {
				if (mi.process_multipart(reqq.processing_token, req, mul, general_buffer ) == module::mss::waiting) return status::idle;
				general_buffer.clear();
				switch (mwm) {
...
blueshift::protocol::status blueshift::protocol::do_res_finalize() {
	...
	if (mi.finalize_response(reqq.processing_token, req, res, resq) == module::mss::waiting) return status::idle;
	mi.cleanup(reqq.processing_token);
	reqq.processing_token = nullptr;
