...
void blueshift::http::request::set_buffer(char const * buf, size_t buf_len) {
	hbuf_len = buf_len;
...
void blueshift::http::request::parse() {
	...
	method = cur;
	if (strcmp(cur, "GET")) {
		status_ = status::method_not_allowed;
		return;
	}
	
	cur = next_token(cur, max_cur);
	if (cur == max_cur) {
		status_ = status::bad_request;
		return;
	}
	path = cur;
	...
	if (strcmp(cur, "HTTP/1.1")) {
		status_ = status::http_version_not_supported;
		return;
	}
